...
exports.login = async (req, res) => {
  ...
  try {
    const doc = await registerModel.findOne({
      mobile: mobileNo,
      password: password,
    });
    if (doc) {
      ...
      const errorResponse = await ApiResponse.error("Invalid credentials", 555);
      // Encrypt the error response
...
exports.logout = async (req, res) => {
  try {
    ...
    await registerModel.updateOne({ _id: req.user._id }, { token: "" });
    const successResponse = await ApiResponse.success(
      null,
      "Logout successful!",
      200
    ...
    const errorResponse = await ApiResponse.error(
